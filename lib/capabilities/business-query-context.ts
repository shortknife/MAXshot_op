...
export function parseBusinessFilters(rawQuery: string): BusinessFilters {
  ...
  const chain = chains.find(([k]) => containsKeyword(text, k))?.[1]
  const protocol = protocols.find(([k]) => containsKeyword(text, k))?.[1]
  return {
