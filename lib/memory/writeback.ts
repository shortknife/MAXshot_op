...
export function buildWritebackEvents(input: {
  ...
  return [
    { timestamp: now, event_type: 'memory_writeback_requested', data: { ...base, status: 'pending_confirmation' } },
    { timestamp: now, event_type: 'memory_writeback_approved', data: { ...base, status: 'confirmed' } },
    { timestamp: now, event_type: 'memory_writeback_written', data: { ...base, status: 'completed' } },
  ]
}
