...
### 3.5 OpenClaw Growth Engine 在多 Unit 下的形态

各 v5.1 Unit 可配置 **OpenClaw Growth Engine**（1 主 3 子：社媒/BD 执行层）。v6.0 阶段：每 Unit 可拥有独立 Growth 实例或共享实例+workspace 隔离；**Fleet Dashboard** 可展示各 Unit 的 Growth 健康度、AIT 消耗、内容产出；**Insight Exchange** 除 SQL 模板外，可纳入脱敏后的内容策略 Pattern（如「周五 meme 效果好」）。融合说明见：`LEO_Decision_Layer/4.Working/产品架构_v5.1_v6.0_与_OpenClaw_Growth_Engine_融合说明_2026-03.md`。
