...
> **版本**: v1.0  
> **目的**: Supabase SQL Editor 一次性执行完整 DDL
...
1. **打开**：https://supabase.com/dashboard/project/[project-ref]/editor
2. **创建新查询窗口**
3. **复制**：粘贴上述完整 DDL 脚本
4. **点击 "Run"**
5. **等待执行完成**
6. **验证表结构**：
   - `tasks_op` 表存在？
   - `task_executions_op` 表存在？
   - `agent_memories_op` 表存在？
   - 所有 6 个索引存在？
7. RLS 策略已配置？
...
- ✅ Service role can manage memories
