...
export function buildFailureOutput(reason: string, scope: Scope, queryContract?: QueryContractV2): CapabilityOutput {
  const criticDecision = evaluateBusinessQueryContract({ queryContract, rows: [] })
  return {
...
export function buildFreeformSuccessOutput(params: {
  ...
  const { freeform, filters, queryContract, memoryRuntime, semanticDefaults, sourcePolicy, followUpPolicy } = params
  const criticDecision = evaluateBusinessQueryContract({ queryContract, rows: freeform.rows })
  return {
    capability_id: 'business_data_query',
    capability_version: '1.0',
    status: 'success',
...
export function buildSuccessOutput(params: {
  ...
  const criticDecision = evaluateBusinessQueryContract({ queryContract, rows: scopedRows })
