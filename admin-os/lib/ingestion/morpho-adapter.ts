...
interface MorphoCleanedPayload {
  isSuccess?: boolean;
  status?: string;
  data?: {
    id?: string | number;
    ...
    subExecutionLogs?: MorphoSubExecution[];
  };
  _cleaned_meta?: Record<string, unknown>;
}

interface MorphoStep11Shape {
  json_data?: Record<string, unknown>;
  cleaned_data?: MorphoCleanedPayload;
}

export interface MorphoAdaptResult {
...
function firstNodeJson(subExec: MorphoSubExecution | undefined, nodeName: string): Record<string, unknown> | undefined {
  return subExec?.data?.resultData?.runData?.[nodeName]?.[0]?.data?.main?.[0]?.[0]?.json;
}

function extractMarkets(currentInfo?: MorphoCurrentInfo): CanonicalMarketRow[] {
...
function extractAllocations(currentInfo?: MorphoCurrentInfo): CanonicalAllocationRow[] {
  const chainName = asString(currentInfo?.chainName);
  const block = currentInfo?.allocationAndMarket?.[0];
...
function extractRebalanceDecision(rebalanceCheck?: Record<string, unknown>): CanonicalRebalanceDecision | null {
  ...
  const reason = asString(rebalanceCheck.reason) || asString(rebalanceCheck.rebalanceReason) || 'No rebalancing needed';
...
function normalizeSource(input: MorphoCleanedPayload | MorphoStep11Shape): {
  cleaned: MorphoCleanedPayload;
  jsonData: Record<string, unknown> | null;
} {
  if ('cleaned_data' in input || 'json_data' in input) {
    return {
      cleaned: (input as MorphoStep11Shape).cleaned_data || {},
      jsonData: (input as MorphoStep11Shape).json_data || null,
    };
  }

  return {
    cleaned: input as MorphoCleanedPayload,
    jsonData: null,
  };
}

export function adaptMorphoPayload(input: MorphoCleanedPayload | MorphoStep11Shape): CanonicalIngestionPayload {
  ...
  for (const subExec of subExecutions) {
    ...
    const currentInfo = firstNodeJson(subExec, 'Current Info') as MorphoCurrentInfo | undefined;
  ...
  return {
    ...
    source_workflow_name: 'morpho-clean-log',
...
export async function adaptAndValidateMorphoPayload(
  input: MorphoCleanedPayload | MorphoStep11Shape,
  db?: QuarantineWriter,
): Promise<MorphoAdaptResult> {
  const payload = adaptMorphoPayload(input);
