...
export function hasYieldGranularity(text: string): boolean {
  return /日均|每天|每日|每小时|小时|中午|12点|实时|最新|平均|avg|daily|hourly/.test(text)
}
...
export function checkClarificationNeeded(params: {
  ...
  const timeWindowDays = Number(params.extractedSlots?.time_window_days)
  ...
  const hasResolvedTime =
    hasTimeWindow(text) || Boolean(dateFrom) || Boolean(dateTo) || (Number.isFinite(timeWindowDays) && timeWindowDays > 0)
  const hasResolvedAggregation =
    hasYieldGranularity(text) ||
    /(最高|peak|\bmax\b|最低|lowest|\bmin\b|平均|均值|avg|average|实时|最新|real-time|realtime)/.test(text) ||
