...
export function buildBusinessHighlights(rows, scope, queryContract) {
  const list = Array.isArray(rows) ? rows : []
  ...
  if (queryContract?.question_shape === 'ranking_by_dimension' && typeof list[0]?.dimension_value !== 'undefined') {
    const top = list[0] || {}
    const dimLabel = queryContract?.ranking_dimension === 'protocol' ? 'Top 协议' : 'Top 链'
    return [
      { label: '分组数', value: String(list.length) },
      { label: dimLabel, value: String(top.dimension_value || '-') },
      { label: 'Top 1 均值 APY', value: `${Number(top.avg_apy_pct || 0).toFixed(2)}%` },
    ]
  }
  if (queryContract?.question_shape === 'top_bottom_in_day' && typeof list[0]?.rank_type !== 'undefined') {
    const highest = list.find((r) => String(r.rank_type) === 'highest') || list[0] || {}
    const lowest = list.find((r) => String(r.rank_type) === 'lowest') || list[list.length - 1] || {}
    return [
      { label: '结果条数', value: String(list.length) },
      { label: '最高市场', value: String(highest.market_name || '-') },
      { label: '最高 APY', value: `${Number(highest.apy_pct || 0).toFixed(2)}%` },
      { label: '最低市场', value: String(lowest.market_name || '-') },
      { label: '最低 APY', value: `${Number(lowest.apy_pct || 0).toFixed(2)}%` },
    ]
  }
  if (queryContract?.metric === 'tvl' && typeof list[0]?.avg_daily_tvl !== 'undefined') {
    const tvls = list.map((r) => Number(r.avg_daily_tvl)).filter((n) => Number.isFinite(n))
    const avgTvl = tvls.length ? (tvls.reduce((a, b) => a + b, 0) / tvls.length).toFixed(2) : '-'
    return [
      { label: '天数', value: String(list.length) },
      { label: '平均日TVL', value: String(avgTvl) },
    ]
  }
  if (typeof list[0]?.rank_type !== 'undefined' && typeof list[0]?.apy_pct !== 'undefined') {
    const highest = list.find((r) => String(r.rank_type) === 'highest') || list[0] || {}
    const lowest = list.find((r) => String(r.rank_type) === 'lowest') || list[list.length - 1] || {}
    return [
      { label: '结果条数', value: String(list.length) },
  ...
  if (typeof list[0]?.avg_daily_tvl !== 'undefined') {
    const tvls = list.map((r) => Number(r.avg_daily_tvl)).filter((n) => Number.isFinite(n))
    const avgTvl = tvls.length ? (tvls.reduce((a, b) => a + b, 0) / tvls.length).toFixed(2) : '-'
    const high = [...list].sort((a, b) => Number(b.avg_daily_tvl || 0) - Number(a.avg_daily_tvl || 0))[0] || {}
    const low = [...list].sort((a, b) => Number(a.avg_daily_tvl || 0) - Number(b.avg_daily_tvl || 0))[0] || {}
    return [
      { label: '天数', value: String(list.length) },
      { label: '平均日TVL', value: String(avgTvl) },
      { label: '最高日', value: `${String(high.day_local || '-')} (${Number(high.avg_daily_tvl || 0).toFixed(2)})` },
      { label: '最低日', value: `${String(low.day_local || '-')} (${Number(low.avg_daily_tvl || 0).toFixed(2)})` },
    ]
  }
  if (s === 'yield') {
    if (typeof list[0]?.rank_type !== 'undefined' && typeof list[0]?.apy_pct !== 'undefined') {
...
/**
 * @param {{ type?: any, summary?: any, rows?: any, draft?: any, error?: any, meta?: any }} params
 */
export function buildUserOutcome({ type, summary, rows = [], draft = null, error = null, meta = {} } = {}) {
  return {
    type,
    summary,
    rows,
