...
import { POST } from '@/app/api/marketing/feedback/route'

describe('marketing feedback route entry enforcement', () => {
  it('returns 403 when execution customer scope is denied', async () => {
    ...
    const res = await POST(new Request('http://localhost/api/marketing/feedback', {
      method: 'POST',
