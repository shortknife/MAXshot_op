...
import type { CustomerAuthDefaultExperience, CustomerPolicyEvidence, CustomerRuntimePolicyMeta } from '@/lib/customers/runtime-policy'
...
export type IdentitySessionResult = IdentitySession
...
export function getSessionDefaultEntryPath(session: IdentitySession | null | undefined): string {
  return session?.customer_policy_evidence?.default_entry_path || session?.customer_runtime_policy?.default_entry_path || '/chat'
}

export function isSurfaceAllowed(session: IdentitySession | null | undefined, surface: string | null | undefined): boolean {
  const normalizedSurface = String(surface || '').trim()
  if (!normalizedSurface) return true
  const focusedSurfaces = session?.customer_policy_evidence?.focused_surfaces || []
  if (!focusedSurfaces.length) return true
  return focusedSurfaces.includes(normalizedSurface)
}
...
export async function requestEmailChallenge(email: string) {
  ...
  return { success: true as const, challenge: data.challenge as EmailChallenge, customer_runtime_policy: (data.challenge?.customer_runtime_policy as CustomerRuntimePolicyMeta | null | undefined) || null, customer_policy_evidence: (data.challenge?.customer_policy_evidence as CustomerPolicyEvidence | null | undefined) || null, auth_default_experience: (data.challenge?.auth_default_experience as CustomerAuthDefaultExperience | null | undefined) || null }
}
...
export async function verifyEmailCode(email: string, challengeId: string, code: string) {
  ...
  const session: IdentitySessionResult = { ...(data.session as IdentitySession), auth_posture: (data.auth_posture as AuthPostureMeta | null | undefined) || null, auth_default_experience: (data.auth_default_experience as CustomerAuthDefaultExperience | null | undefined) || null, customer_runtime_policy: (data.customer_runtime_policy as CustomerRuntimePolicyMeta | null | undefined) || null, customer_policy_evidence: (data.customer_policy_evidence as CustomerPolicyEvidence | null | undefined) || null }
...
export async function requestWalletChallenge(walletAddress: string) {
  ...
  const data = await parseJson(res)
  if (!res.ok || data.success !== true) return { success: false as const, error: String(data.error || 'challenge_failed') }
  return { success: true as const, challenge: data.challenge as WalletChallenge, customer_runtime_policy: (data.challenge?.customer_runtime_policy as CustomerRuntimePolicyMeta | null | undefined) || null, customer_policy_evidence: (data.challenge?.customer_policy_evidence as CustomerPolicyEvidence | null | undefined) || null, auth_default_experience: (data.challenge?.auth_default_experience as CustomerAuthDefaultExperience | null | undefined) || null }
}
...
export async function verifyWalletSignature(walletAddress: string, challenge: WalletChallenge) {
  try {
    ...
    const session: IdentitySessionResult = { ...(data.session as IdentitySession), auth_posture: (data.auth_posture as AuthPostureMeta | null | undefined) || null, auth_default_experience: (data.auth_default_experience as CustomerAuthDefaultExperience | null | undefined) || null, customer_runtime_policy: (data.customer_runtime_policy as CustomerRuntimePolicyMeta | null | undefined) || null, customer_policy_evidence: (data.customer_policy_evidence as CustomerPolicyEvidence | null | undefined) || null }
    setStoredSession(session)
    return { success: true as const, session, signature }
  } catch (error) {
