...
export async function POST(req: NextRequest) {
  try {
    ...
    const pendingConfirmation = require_confirmation === true;
    const initialStatus = pendingConfirmation ? 'pending_confirmation' : 'created';
    const auditLog = {
    ...
    const { error: execError } = await supabase
      ...
      .insert({
        ...
        reason_for_pending: pendingConfirmation ? (reason_for_pending || 'side_effect') : null,
        confirmation_request: pendingConfirmation ? (confirmation_request || null) : null,
        status: initialStatus,
        audit_log: auditLog,
    ...
    return NextResponse.json({
      success: true,
      task_id: createdTaskId,
      execution_id: executionId, // EM-T1 新增：返回给 Alex
