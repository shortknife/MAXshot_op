import { AuditLog, ExecutionResult } from '../types'
...
export function buildAttribution(input: {
  ...
  for (const out of outputs as any[]) {
    ...
    const status = out?.status === 'success' ? 'contributed' : 'failed'
  ...
  const evidenceSourcesCount = outputs.reduce((sum, out: any) => sum + (out?.evidence?.sources?.length || 0), 0)
