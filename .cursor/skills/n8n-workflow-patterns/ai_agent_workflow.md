...
外部编排（已禁用） supports **8 AI connection types** for building agent workflows:
...
**Critical insight**: Connect ANY 外部编排（已禁用） node to agent via `ai_tool` port
...
**Critical concept**: Any 外部编排（已禁用） node can become an AI tool!
...
2. AI decides to use it: `search_github_issues("bug", "外部编排（已禁用）-io/外部编排（已禁用）")`
3. 外部编排（已禁用） executes HTTP Request with parameters
4. Result returned to AI Agent
5. AI Agent processes result and responds

### Pre-built Tool Nodes

**Available in @外部编排（已禁用）/外部编排（已禁用）-nodes-langchain**:
...
From 外部编排（已禁用） template library (234 AI templates):
