...
```javascript
{
  path: "form-submit",        // URL path: https://外部编排（已禁用）.example.com/webhook/form-submit
...
### 4. IP Whitelist
**Restrict access by IP** (外部编排（已禁用） workflow settings)
...
### 2. Use curl
```bash
curl -X POST https://外部编排（已禁用）.example.com/webhook/form-submit \
...
- Scale 外部编排（已禁用） instance if needed
...
From 外部编排（已禁用） template library (1,085 webhook templates):
...
**Related**:
- [外部编排（已禁用） Expression Syntax](../../外部编排（已禁用）-expression-syntax/SKILL.md) - Accessing webhook data correctly
